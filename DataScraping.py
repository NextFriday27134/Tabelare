...
HEADERS = ["Zona", "Pret", 'Suprafață', 'Forma de proprietate', 'Numărul de camere', 'Stare', 'Etaj',
           'Balcon/grădină/terasă', 'Chirie', 'Garaj/loc de parcare', 'Vizionare la distanță', 'Încălzire',
           'Tip proprietate', 'Tip vânzător', 'Liber de la', 'Anul construcției', 'Tip clădire', 'Tip geamuri', 'Lift',
           'Media', 'Siguranță', 'Facilități', 'Informații suplimentare',
           'Material de construcție', "Titlu", "Descriere"]
ALL_APARTMENTS_URLS = []
...
def get_new_soup(page_number):
    new_url = REAL_ESTATE_SITE_SALE + f'&page={page_number}'
...
def get_all_apartments_url(soup_object: BeautifulSoup):
...
def save_data_as_excel_table(table_name):
    wb = Workbook()
    ws = wb.active
    ws.append(HEADERS)
    for url in ALL_APARTMENTS_URLS:
        ...
        try:
            ...
            # pare o eroare de la server ar merge sa pun sa incerce din nou pt intrarile respective
            print(new_url)
    ...
    tab = Table(displayName=table_name, ref=f"A1:{get_column_letter(len(HEADERS))}{len(ALL_APARTMENTS_URLS)+1}")
...
if __name__ == "__main__":
    ...
    get_all_apartments_url(soup)
    save_data_as_excel_table(table_name='apartamente_de_vanzare')

    # List of apartments for rent.

    soup = make_it_beauty(get_site_html_code(REAL_ESTATE_SITE_RENT))
    get_all_apartments_url(soup)
    save_data_as_excel_table(table_name='apartamente_de_inchiriat')
